...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      const checkAuth = () => {
        if (!env.AUTH_KEY) {
          return new Response('Server Configuration Error: AUTH_KEY is not set on the Cloudflare Worker.', { status: 500, headers: getCorsHeaders(request, env) });
        }
        const header = request.headers.get('Authorization');
        if (header !== `Bearer ${env.AUTH_KEY}`) {
          return new Response('Unauthorized: Invalid or missing API Auth Key', { status: 401, headers: getCorsHeaders(request, env) });
        }
        return null;
      };
      ...
      if (path === '/create' && request.method === 'POST') {
        ...
        if (!body.blob_key) {
          return new Response('Missing blob_key', { status: 400, headers: getCorsHeaders(request, env) });
        }
      ...
      if (path.startsWith('/sync/') && request.method === 'PUT') {
        ...
        if (!userId) return new Response('Bad request', { status: 400, headers: getCorsHeaders(request, env) });
      ...
      return new Response('Internal Server Error', { status: 500, headers: getCorsHeaders(request, env) });
    }
  },
};
